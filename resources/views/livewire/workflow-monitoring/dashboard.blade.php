                                ...
                                @forelse($rules as $rule)
                                    <div
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <div class="text-xs text-gray-500 mt-0.5">Trigger: {{ $rule->trigger_event }}
                                            </div>
                                        </div>
                                        <div class="text-right px-4">
                                            <div class="text-sm font-bold text-gray-900">{{ $rule->logs_count }}</div>
                                            <div class="text-[10px] text-gray-400 font-medium uppercase tracking-tighter">
                                                Executions</div>
                                        </div>
                                        <div class="ps-4">
                                            <div
                                                class="h-2.5 w-2.5 rounded-full {{ $rule->is_active ? 'bg-green-500' : 'bg-gray-300' }}">
                                            </div>
                                        </div>
                                    </div>
                                @empty
                                        ...
                                        <li>
                                            <div class="relative pb-8">
                                                ...
                                                <div class="relative flex space-x-3">
                                                    <div>
                                                        <span
                                                            class="h-8 w-8 rounded-full {{ $log->status === 'success' ? 'bg-indigo-100 text-indigo-600' : 'bg-red-100 text-red-600' }} flex items-center justify-center ring-8 ring-white">
                                                    ...
                                                    <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                                        <div>
                                                            <p class="text-sm text-gray-500 font-medium">
                                                                <span
                                                                    class="font-bold text-gray-900">{{ $log->rule->name ?? 'System Event' }}</span>
                    ...
                    <div class="bg-indigo-600 rounded-2xl p-8 text-white shadow-xl shadow-indigo-100">
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between items-center text-xs border-t border-indigo-500 pt-4">
                                ...
                                <span class="font-black">{{ $rules->where('is_active', true)->count() }}</span>
                            </div>
                            <div class="flex justify-between items-center text-xs border-t border-indigo-500 pt-4">
                                <span class="opacity-70">Total Executions</span>
                                <span class="font-black">{{ $rules->sum('logs_count') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
