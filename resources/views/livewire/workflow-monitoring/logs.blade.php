<x-app-container>
    ...
    <x-toolbar class="mb-6" variant="subtle">
        <x-slot name="left">
            <x-input wire:model.live.debounce.300ms="search" placeholder="Search rules…" aria-label="Search workflow rules" class="w-full sm:w-96" />
                ...
                <x-slot name="body">
                    @forelse($logs as $log)
                        <tr class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4">
                                <span class="text-sm font-semibold text-gray-900 tracking-tight">{{ $log->rule->name ?? 'System Event' }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-widest 
                                    {{ $log->status === 'success' ? 'bg-indigo-100 text-primary' : 'bg-red-100 text-red-600' }}">
                                    {{ $log->status }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                 <button wire:click="viewDetails('{{ $log->id }}')" class="text-xs font-semibold text-primary uppercase tracking-widest hover:text-indigo-800 transition">View Details</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
            ...
            <div class="fixed inset-y-0 right-0 pl-10 max-w-full flex">
                <div class="w-screen max-w-2xl bg-white shadow-2xl rounded-l-[3rem] overflow-hidden flex flex-col">
                    <div class="p-8 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-gray-50 to-white">
                        <div>
                            <span class="text-[10px] font-semibold uppercase tracking-widest text-primary italic">Workflow Run #{{ substr($selectedLog->id, 0, 8) }}</span>
                            <h2 class="text-2xl font-semibold text-gray-900 tracking-tighter">{{ $selectedLog->rule->name ?? 'Internal Trigger' }}</h2>
                        </div>
                        <button wire:click="closeInspector" class="p-2 rounded-xl hover:bg-gray-100 transition" aria-label="Close inspector">
                    ...
                    <div class="flex-1 overflow-y-auto p-10 space-y-10">
                        <section>
                            <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-6 italic">Execution Context</h3>
                            <div class="grid grid-cols-2 gap-6">
                                <div class="p-6 bg-gray-50 rounded-2xl">
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Status</p>
                                    <span class="text-sm font-semibold {{ $selectedLog->status === 'success' ? 'text-primary' : 'text-red-600' }} uppercase italic">{{ $selectedLog->status }}</span>
                                </div>
                                <div class="p-6 bg-gray-50 rounded-2xl">
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Executed At</p>
                                    <span class="text-sm font-semibold text-gray-900 italic">{{ $selectedLog->created_at->format('M d, Y H:i:s') }}</span>
                                </div>
                            </div>
                        </section>

                        <section>
                            <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-6 italic">Trigger Payload</h3>
                        ...
                        @if($selectedLog->status === 'failed')
                            <section class="p-8 bg-red-50 rounded-[2.5rem] border-2 border-red-100">
                                <h3 class="text-xs font-semibold text-red-400 uppercase tracking-widest mb-3">Error Diagnosis</h3>
