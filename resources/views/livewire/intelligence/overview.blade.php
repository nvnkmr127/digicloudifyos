<div class="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col items-start justify-between gap-4 mb-10 md:flex-row md:items-center">
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight text-slate-900">Intelligence Overview</h1>
            <p class="mt-1 text-slate-500">Agency-wide performance monitoring and AI-driven optimizations.</p>
        </div>
        <div class="flex gap-4">
            <a href="{{ route('intelligence.briefing') }}" class="px-5 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition">View Morning Briefing</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2 space-y-8">
            <div class="p-8 bg-white border border-slate-200 rounded-3xl shadow-sm">
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-xl font-black text-slate-900">Client Portfolio Health</h3>
                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Active Clients: {{ $clients->count() }}</div>
                </div>

                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    @foreach($clients as $c)
                        <a href="{{ route('intelligence.client', $c->id) }}" class="flex items-center p-4 transition border border-slate-100 rounded-2xl bg-slate-50 hover:bg-white hover:shadow-md hover:border-indigo-100 group">
                            <div class="flex items-center justify-center w-12 h-12 mr-4 font-black rounded-xl {{ $c->latestHealthScore?->getScoreBadgeClass() ?? 'bg-slate-200 text-slate-400' }}">
                                {{ $c->latestHealthScore?->overall_score ?? '?' }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <h4 class="text-sm font-bold truncate text-slate-900 group-hover:text-indigo-600">{{ $c->name }}</h4>
                                <div class="flex items-center mt-1">
                                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $c->industry ?? 'Other' }}</span>
                                    @if($c->latestHealthScore?->trend === 'improving')
                                        <span class="ml-2 text-emerald-500 text-[10px] font-bold">↑Improving</span>
                                    @elseif($c->latestHealthScore?->trend === 'declining')
                                        <span class="ml-2 text-red-500 text-[10px] font-bold">↓Declining</span>
                                    @endif
                                </div>
                            </div>
                            <div class="ml-2 opacity-0 group-hover:opacity-100 transition text-indigo-500">
            ...
            <!-- Recent AI Insights -->
            <div class="p-8 bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl relative overflow-hidden">
        ...
        <div class="space-y-8">
            <div class="p-8 bg-white border border-slate-200 rounded-3xl shadow-sm">
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-lg font-black text-slate-900">Live Alerts</h3>
                    <a href="{{ route('intelligence.alerts') }}" class="text-[10px] font-bold text-indigo-600 uppercase tracking-widest hover:underline">View All</a>
                </div>

                <div class="space-y-4">
                    @forelse($topAlerts as $alert)
                        <div class="flex items-start gap-4 p-4 border rounded-2xl border-slate-100 bg-slate-50">
                            ...
                            <div class="min-w-0">
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $alert->client->name }}</p>
                                <p class="mt-0.5 text-xs font-bold text-slate-800">{{ $alert->metric_name }} shifted by {{ abs(round($alert->deviation_percentage)) }}%</p>
                                <p class="mt-1 text-[10px] font-medium text-slate-500 italic">{{ str_replace('_', ' ', $alert->anomaly_type) }} detected {{ $alert->detected_at->diffForHumans() }}</p>
                            </div>
                        </div>
                    @empty
            ...
            <div class="p-8 bg-white border border-slate-200 rounded-3xl shadow-sm">
                <h3 class="mb-6 text-xs font-black tracking-widest text-slate-400 uppercase">Intelligence Pipeline</h3>
                <div class="space-y-6">
                    <div class="flex items-center gap-4">
                        <div class="w-8 h-8 flex items-center justify-center bg-emerald-100 text-emerald-700 rounded-lg">
