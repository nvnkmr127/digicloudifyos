<x-app-container>
    <x-page-header title="Inbound Webhook Mappings" />
    ...
    @if (! $hasTable)
        <x-card>
            <p class="text-sm text-text-muted">Run migrations to enable inbound mapping management.</p>
        </x-card>
    @else
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <x-card class="lg:col-span-1">
                <h3 class="text-base font-semibold text-text-primary mb-4">{{ $mappingId ? 'Edit Inbound Mapping' : 'Create Inbound Mapping' }}</h3>
                <form wire:submit.prevent="save" class="space-y-4">
                    ...
                    <div>
                        <label class="text-xs font-semibold text-text-muted uppercase">Source Key</label>
                    ...
                    <div>
                        <label class="text-xs font-semibold text-text-muted uppercase">Target Field</label>
                    ...
                    <div>
                        ...
                        <input wire:model.defer="transform_rule" type="text" class="mt-1 w-full rounded-md border-gray-300 text-sm" placeholder="none, lowercase, date_parse" />
                    </div>

                    <div>
                        <label class="text-xs font-semibold text-text-muted uppercase">Inbound Endpoint</label>
                        <select wire:model.defer="inbound_webhook_id" class="mt-1 w-full rounded-md border-gray-300 text-sm">
                            <option value="">Any endpoint</option>
                            @foreach($inboundWebhooks as $webhook)
            ...
            <x-card class="lg:col-span-2">
                <h3 class="text-base font-semibold text-text-primary mb-4">Inbound Mapping Rules</h3>
                <div class="overflow-x-auto rounded-lg border border-gray-100">
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-text-muted uppercase">Name</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-text-muted uppercase">Source</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-text-muted uppercase">Target</th>
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white">
                            ...
                            @empty
                                <tr>
                                    <td class="px-4 py-4 text-sm text-text-muted" colspan="5">No inbound mappings configured.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
