...
class PerformanceMonitorService
{
    ...
    protected function autoResolveAnomalies(string $clientId, string $orgId, string $date): void
    {
        ...
        foreach ($unresolved as $anomaly) {
            /** @var \App\Models\PerformanceAnomaly $anomaly */
    ...
    public function calculateHealthScore(string $clientId, string $orgId, string $date): void
    {
        ...
        foreach ($anomalies as $anomaly) {
            ...
            if (in_array($anomaly->anomaly_type, ['ctr_drop', 'cpc_spike', 'roas_decline'])) $adPerf -= $deduction;
            if (in_array($anomaly->anomaly_type, ['lead_drop'])) $conversion -= $deduction;
            if (in_array($anomaly->anomaly_type, ['engagement_drop'])) $organic -= $deduction;
            if (in_array($anomaly->anomaly_type, ['budget_overrun', 'budget_underpace'])) $budget -= $deduction;
        }
        ...
        $trend = 'stable';
        if ($lastScore) {
            if ($overall > ($lastScore->overall_score + 5)) $trend = 'improving';
            elseif ($overall < ($lastScore->overall_score - 5)) $trend = 'declining';
        }
