    ...
    public function render()
    {
        ...
        $stats = [
            ...
            'client_health_grid' => Client::with(['latestHealthScore'])
                ...
                ->limit(6)
                ->get(),
            'morning_briefing_preview' => \App\Models\DailyBriefing::with(['actionItems' => fn($q) => $q->limit(3)->orderBy('sort_order')])
                ->where('organization_id', $orgId)
                ->whereDate('briefing_date', today())
                ->first(),
            'recent_anomalies' => \App\Models\PerformanceAnomaly::with('client')
                ->where('organization_id', $orgId)
                ->unresolved()
